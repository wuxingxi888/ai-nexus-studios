...
const MAX_COLORS = 8;
const MAX_DPR = 1.5;
const DEFAULT_COLORS = ["#2b3ebe", "#6d7cff", "#c9d4ff"];

const getRendererDpr = () => Math.min(window.devicePixelRatio || 1, MAX_DPR);
...
export function LightfallBackground({
    ...
    useEffect(() => {
        ...
        const loop = (t: number) => {
            ...
            if (!pausedRef.current && visibleRef.current && programRef.current && meshRef.current) {
        ...
        const syncLoop = () => {
            if (document.visibilityState === "hidden" || pausedRef.current || !visibleRef.current) {
                stopLoop();
                return;
            }
            if (rafRef.current || resumeTimerRef.current) return;
